...
When you have collected comprehensive information, use the sendToAgent tool to submit the data.`,
    tools: {
      sendToAgent: {
        description: 'Send collected comprehensive health data to the research agents for analysis. Only use this when you have gathered thorough information about the user\'s hormone-related health concerns.',
        parameters: z.object({
          user_data: z.object({
            ...
            tracking_level: z.string().describe('How closely they track (not at all/somewhat/very closely)'),
            ...
            cycle_regularity: z.string().describe('Regular/irregular cycle info'),
        ...
        execute: async ({ user_data }) => {
          ...
          const dataWithId = {
            ...user_data,
            user_id: user_data.user_id || `user_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
